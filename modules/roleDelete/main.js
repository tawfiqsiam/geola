module.exports = async (client, role) => {
    ...
    const modules = client.modules.roleDelete;

    //Get server data
    const serverData = await models.servers.findById(role.guild.id);

    //Log
    const logChannel = role.guild.channels.get(serverData.logChannels && serverData.logChannels.roleDelete);
    if (logChannel) {
    ...
    //Stats
    await _.stats(client, "Roles Deleted");
};