module.exports = async (client, emoji) => {
    ...
    const modules = client.modules.emojiDelete;

    //Get server data
    const serverData = await models.servers.findById(emoji.guild.id);

    //Log
    const logChannel = emoji.guild.channels.get(serverData.logChannels && serverData.logChannels.emojiDelete);
    if (logChannel) {
    ...
    //Stats
    await _.stats(client, "Emojis Deleted");
};